	<div id="sly" class="ei-slider">
		<ul class="ei-slider-large">
			<li>
				<img src="/c/large/www-adventures-kota-kinabalu.jpg" alt="Kota Kinabalu, the capital of the West Coast Division of Sabah"/>
			</li>
		</ul>
	</div>
	
	<nav></nav><section>	
			
		<div class="container hd lh">

			<div class="two columns"></div>

			<div class="sixteen columns big_title">
				<ul class="brc">
					<li><a href="/home">Home</a></li>
					<li><a href="/our-adventures">Our Adventures</a></li>
					<li>Kota Kinabalu</li>
				</ul>
				<h1>Kota Kinabalu</h1>
				<p>
					Kota Kinabalu is the main gateway into the Malaysian Borneo with a myriad of international air connections. Most travelers spend the beginning or end of their trip with few nights in Kota Kinabalu – just to get their bearings and trip planning, including essentials, before heading off into the areas that you came to Borneo for. If you happen to have a few days to spare in Kota Kinabalu, have a look at our selected programs below to keep you occupied. 
				</p>	
				<p>
					You can go diving or kayaking in the Tunku Abdul Rahman Park, cycling in the countryside, white water rafting, visit the market with a cooking class program or spend the night in a lovely home located in a fruit orchard up in the mountain range of Kota Kinabalu.
				</p>
			</div>	
			
			<div class="clear"></div>	

			<div class="one columns"></div>

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/cycling-programs">
						<img src="/c/adventures/cycling-programs.jpg" class="img1" alt="Cycling Programs"/>
						<h2>Cycling Programs</h2>
<!-- 						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div> -->
					</a>			
				</div>
			</div>		

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/diving-and-snorkeling">
						<img src="/c/adventures/diving-tunku-abdul-rahman-park.jpg" class="img1" alt="Diving &amp; Snorkeling Programs"/>
						<h2>Diving &amp; Snorkeling Programs</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/kayak-and-snorkel-safari">
						<img src="/c/adventures/sea-kayaking.jpg" class="img1" alt="Kayak &amp; Snorkel Safari"/>
						<h2>Kayak &amp; Snorkel Safari</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="one columns"></div>	

			<div class="clear display"></div>	

			<div class="one columns"></div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/momma-house-cooking-class">
						<img src="/c/adventures/momma-house-cooking-class.jpg" class="img1" alt="Momma's House Cooking Class"/>
						<h2>Momma's House Cooking Class</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>		

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/white-water-rafting">
						<img src="/c/adventures/kiulu-white-water-rafting.jpg" class="img1" alt="White Water Rafting"/>
						<h2>White Water Rafting</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a>
						<img src="/c/adventures/coming-soon.jpg" class="img1" alt="Borneo Orchard House"/>
						<h2>Borneo Orchard House</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>
			
			<div class="clear"></div>	

			<div class="one columns"></div>			

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/borneo-backpackers">
						<img src="/c/adventures/borneo-backpackers.jpg" class="img1" alt="Borneo Backpackers"/>
						<h2>Borneo Backpackers</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>		

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/gaya-island-resort">
						<img src="/c/adventures/gaya-island-resort.jpg" class="img1" alt="Gaya Island Resort"/>
						<h2>Gaya Island Resort</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/grandis-hotel">
						<img src="/c/adventures/grandis-hotel.jpg" class="img1" alt="Grandis Hotel"/>
						<h2>Grandis Hotel</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="one columns"></div>	

			<div class="clear display"></div>	

			<div class="one columns"></div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/hotel-sixty3">
						<img src="/c/adventures/hotel-sixty3.jpg" class="img1" alt="Hotel Sixty3"/>
						<h2>Hotel Sixty3</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>		

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/hyatt-regency">
						<img src="/c/adventures/hyatt-regency.jpg" class="img1" alt="Hyatt Regency"/>
						<h2>Hyatt Regency</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/jesselton-hotel">
						<img src="/c/adventures/jesselton-hotel.jpg" class="img1" alt="Jesselton Hotel"/>
						<h2>Jesselton Hotel</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>	
			
			<div class="clear"></div>	

			<div class="one columns"></div>	

			<div class="six columns product_box">
				<div class="img_boxing">
					<a href="./our-adventures/kota-kinabalu/kk-suites">
						<img src="/c/adventures/kk-suites.jpg" class="img1" alt="KK Suites"/>
						<h2>KK Suites</h2>
						<div class="rating accommodation">
							<h3>Dwelling</h3>
						</div>
						<div class="rating difficulty">
							<h3>Difficulty</h3>
						</div>
						<div class="clear"></div>
						<div class="category">
							<h3>Category</h3>
						</div>
					</a>			
				</div>
			</div>
			
			<div class="clear"></div>
			
		</div>
		
	</section>

<script>
	getDetails("Kota Kinabalu");
</script>