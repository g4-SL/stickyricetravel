<!-- Url: our-adventures/kota-kinabalu/momma-house-cooking-class	 -->

	<div id="sly" class="ei-slider">
		<ul class="ei-slider-large">
			<li>
				<img src="c/large/www-adventures-momma-house-cooking-class.jpg" alt="Momma's House Cooking Class"/>
				<div class="ei-title"></div>
			</li>
		</ul>
	</div>
	
	<nav></nav><section>	
			
		<div class="container nd">
		
			<div class="nine columns">
				<h1>Momma's House Cooking Class</h1>
			</div>			

			<div class="seven columns">
				<ul class="brc">
					<li><a href="./home">Home</a></li>
					<li><a href="./our-adventures">Our Adventures</a></li>
					<li><a href="./our-adventures/kota-kinabalu">Kota Kinabalu</a></li>
					<li>Momma's House Cooking Class</li>
				</ul>
			</div>
			
			<div class="clear"></div>
			
			<div class="nine columns">

				<p>
					Get outside the city and into the kitchen with our specialty outdoor cooking classes, located at ‘Momma’s House’, about 15 minutes outside of Kota Kinabalu. This program will take you to a local market with your program host before proceeding to a local home for the cooking lesson.
				</p>

				<h2 class="grunge6">Program</h2>
				<table class="tbl_p">
					<tbody>
						<tr valign="middle">
							<th><img src="c/day1.png" alt="Day 1 - Day One" /></th>
							<td>
								Picked up from your hotel in Kota Kinabalu at 8.30am and straight to the market with your host. You will start your excursion with learning how the locals buy their food in a local market or ‘tamu’. You will see produce – meat, seafood, fruit and vegetable, all grown locally for local consumption. 
								<br /><br />
								We then proceed to Momma’s House in one of the villages in the outskirt of the city. Your host family will treat you like family and share their family recipes to cook like a Sabahan. Sit down with the family and eat lunch at the house after the cooking lesson. Your driver will transfer you back to your hotel in Kota Kinabalu around 3pm.		
							</td>
						</tr>
					</tbody>
				</table>
	
				<h2 class="grunge5">Included/Excluded</h2>	
				<table class="tbl_p">
					<tbody>
						<tr>
							<td class="he" width="50%">What is Included?</td>
							<td class="he" width="50%">What is Not Included?</td>
						</tr>
						<tr>
							<td>
								Return transfers from Kota Kinabalu, drinking water &amp; lunch, cooking lesson and recipe book.
							</td>
							<td>
								Additional beverages, tipping and gratuities for host.
							</td>
						</tr>
						<tr>
						<td class="he" colspan="2">Departure</td>
						</tr>
						<tr>
							<td colspan="2">
								Daily based on a minimum booking of 2pax per departure. Single supplement applies for solo travellers.     
							</td>
						</tr>
					</tbody>
				</table>			
				
			</div>	
			
			<div class="seven columns bp">
				<?php include './a/side-book-now.php'; ?>
				<img src="c/adventures-mommas-house-cooking-class/1-400.jpg" alt="Momma's House Cooking Class" />
				<img src="c/adventures-mommas-house-cooking-class/2-400.jpg" alt="Momma's House Cooking Class" />
				<img src="c/adventures-mommas-house-cooking-class/3-400.jpg" alt="Momma's House Cooking Class" />
				<img src="c/adventures-mommas-house-cooking-class/4-400.jpg" alt="Momma's House Cooking Class" />
				<img src="c/adventures-mommas-house-cooking-class/5-400.jpg" alt="Momma's House Cooking Class" />
				<p><i>*Photo credit to Momma's House Cooking Class</i></p>
			</div>

		</div>
		
	</section>