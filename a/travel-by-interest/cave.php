	<div id="sly" class="ei-slider">
		<ul class="ei-slider-large">
			<li>
				<img src="c/large/www-travel-by-interest-cave.jpg" alt="Gomantong Cave, Sandakan" />
			</li>
		</ul>
	</div>
	
	<nav></nav><section>	
		<div class="container hd lh">

			<div class="one columns"></div>

			<div class="seven columns interest-col">
				<?php $interest_selected = "cave"; include "./helper/sm-show-interest.php" ?>

				<div class="sm-button circle_center vc">
					<a href="./travel-by-interest">Travel by Interest</a>
				</div>
			</div>	

			<div class="eleven columns big_title interest-col">
				<h1>Cave</h1>
				<h4>
					Famous for its intricate cave system, Borneo is the place to explore dark caves and to learn how life cycles in a cave.
				</h4>
			</div>	
			
			<div class="clear"></div>

			<div class="one columns white-space-listing"></div>

			<div class="category-listing">
			</div>

		</div>

	</section>

<script>
	getDetailsBasedOnCategory("Cave");
</script>